...
public class NewHandClassTest {
    ...
    public void accept() {
        ...
        assertEquals("same card", newCard, ((HandClass) newHand).getCardFromDeck(1));
    }
   ...
   @Test
    ...
    public void occurrencesInHand()
    {
        ...
        Hand newHand = new NewHandClass(cards);

        int numberOfOccurences = ((NewHandClass) newHand).occurrencesInHand(c1);
        assertEquals("4", numberOfOccurences, 4);

        newHand.accept(c1);
        newHand.accept(c1);


        int numberOfOccurences1 = ((NewHandClass) newHand).occurrencesInHand(c1);
        assertEquals("6", numberOfOccurences1, 6);


        /* Testing rank */
        int expectedOccurences = 6;
        Rank expectRank = RankEnum.EIGHT;
        assertEquals("same rank", expectedOccurences,
                ((NewHandClass) newHand).occurencesInHand(expectRank));
    }
}